    ...
    def normalize_text(self, text: str) -> str:
        ...
        if re.search(r'[()+\-#$%&*/<=>@~№×≈÷]', text):
            ...
            symbol_dict = {
                ...
                '&': ' and ', "'": ' кавычка ', '(': ' в скобках ', ')': ' скобка закрывается ',
                '*': ' умножить на', '+': ' плюс ', ',': ',', '-': ' минус ', '.': '.', '/': ' разделить на ',
                ...
                '}': ' правая фигурная скобка ', '№': ' номер ', '×': ' умножить на', '÷': ' разделить на ',  '≈': ' примерно ',
            }
            ...
            # Заменяем " +", "+ " на слово "плюс"
            text = re.sub(r'\s*\+\s*', ' плюс ', text)

            # Заменяем " -", "- " на слово "минус"  
            text = re.sub(r'\s*\-\s*', ' минус ', text)
            _logger.debug(f'Текст после замены + и - кроме как внутри слов: {text}')

            # Удаляем все остальные символы
            text = re.sub(f'[^{re.escape(symbols_to_change)}{re.escape(symbols_to_keep)}A-Za-zЁА-Яа-яё\d]', '', text)
